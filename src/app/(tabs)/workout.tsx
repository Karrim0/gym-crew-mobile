...
export default function WorkoutTab() {
  ...
  return (
    <Screen refreshing={refreshing} onRefresh={() => void load(true)} horizontalPadding={16}>
      <AppHeader title={language === "ar" ? "التمرين" : "Workout"} subtitle={language === "ar" ? "ابدأ، سجّل، وكمّل من غير تعطيل." : "Start, log, and keep moving."} />

      {active ? (
        <PhotoHero
          source={workoutVisual(active.exercises[0]?.exercise.workoutType)}
          height={252}
          topRight={<View style={{ paddingHorizontal: 11, paddingVertical: 7, borderRadius: 999, backgroundColor: "rgba(9,10,13,0.76)", borderWidth: 1, borderColor: colors.borderStrong }}><AppText variant="caption" color="primary">{language === "ar" ? "شغّالة" : "LIVE"}</AppText></View>}
        >
          <View style={{ gap: 11 }}>
            <AppText variant="hero" style={{ color: colors.textOnDark }}>{language === "ar" ? "كمّل تمرينتك" : "Continue your workout"}</AppText>
            ...
            <View style={{ flexDirection: rowDirection, alignItems: "center", justifyContent: "space-between" }}><AppText variant="caption" style={{ color: colors.textOnDarkMuted }}>{activeStats.completed}/{activeStats.total} {t("common.sets")}</AppText><AppText variant="bodyStrong" numeric color="primary">{Math.round(activeStats.percent)}%</AppText></View>
            <ProgressBar value={activeStats.percent} />
            <Button onPress={() => router.push(`/workout/${active.id}`)} icon={<Play fill={colors.primaryInk} color={colors.primaryInk} size={20} />}>{language === "ar" ? "افتح وضع التمرين" : "Open Gym Mode"}</Button>
          </View>
        </PhotoHero>
      ) : (
      ...
      <Pressable onPress={() => router.push("/(tabs)/split")} style={({ pressed }) => ({ opacity: pressed ? 0.68 : 1 })}>
        <Card variant="glass" elevated={false} style={{ flexDirection: rowDirection, alignItems: "center", gap: spacing.md }}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}><AppText variant="bodyStrong">{language === "ar" ? "جدولك" : "Your plan"}</AppText><AppText variant="small" color="muted">{language === "ar" ? "عدّل اليوم أو التمارين قبل ما تبدأ" : "Edit the day or exercises before you start"}</AppText></View>
        ...
        {history.map((session) => {
          ...
          return (
            <Pressable key={session.id} accessibilityRole="button" onPress={() => router.push(`/workout-history/${session.id}`)} style={({ pressed }) => ({ opacity: pressed ? 0.68 : 1 })}>
              <Card variant="glass" elevated={false} style={{ flexDirection: rowDirection, alignItems: "center", gap: spacing.md, padding: 10 }}>
